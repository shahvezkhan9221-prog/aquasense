...
function ProfileMenu({ align = "end" }: { align?: "start" | "end" }) {
  ...
  return <div className="profile-menu" onClick={(e) => e.stopPropagation()}>
    ...
    {open && <div className={`profile-dropdown ${align === "start" ? "align-start" : "align-end"}`} role="menu">
      <div className="profile-dropdown-header"><div className="avatar">TO</div><div className="min-w-0"><div className="truncate text-xs font-medium">Tarun Oli</div><div className="truncate text-[10px] text-muted-foreground">User</div></div></div>
...
function FilterDropdown({ value, onChange, options, allLabel, disabled }: { value: string; onChange: (v: string) => void; options: string[]; allLabel: string; disabled?: boolean }) {
  const [open, setOpen] = useState(false)
  useEffect(() => {
    if (!open) return
    const close = () => setOpen(false)
  ...
  return <div className="state-dropdown" onClick={(e) => e.stopPropagation()}>
    <button className="state-dropdown-trigger" onClick={() => !disabled && setOpen((v) => !v)} aria-haspopup="true" aria-expanded={open} disabled={disabled} aria-disabled={disabled}>
      <MapPin className="size-3.5 text-primary" />{value === "All" ? allLabel : value}<ChevronDown className={`size-3.5 transition-transform ${open ? "rotate-180" : ""}`} />
    </button>
    {open && !disabled && <div className="state-dropdown-menu" role="menu">
      {["All", ...options].map((s) => <button key={s} role="menuitem" onClick={() => { onChange(s); setOpen(false) }} className={`state-dropdown-item ${value === s ? "selected" : ""}`}>{s === "All" ? allLabel : s}{value === s && <span className="size-1.5 rounded-full bg-primary shadow-[0_0_10px_#63e6e2]" />}</button>)}
    </div>}
  </div>
}
...
export default function Dashboard() {
  ...
  const states = useMemo(() => Array.from(new Set(locations.map((l) => l.state))).sort(), [])
  const districtsInState = useMemo(() => selectedState === "All" ? [] : Array.from(new Set(locations.filter((l) => l.state === selectedState).map((l) => l.district))).sort(), [selectedState])
  const villagesInState = useMemo(() => locations.filter((l) => (selectedState === "All" || l.state === selectedState) && (selectedDistrict === "All" || l.district === selectedDistrict)), [selectedState, selectedDistrict])

  useEffect(() => { setMetrics(baseMetrics[selectedId]) }, [selectedId])
  useEffect(() => { setSelectedDistrict("All") }, [selectedState])
  useEffect(() => { if (!villagesInState.some((l) => l.id === selectedId)) setSelectedId(villagesInState[0]?.id ?? locations[0].id) }, [selectedState, selectedDistrict])
  ...
  return <div className="relative z-[1] min-h-screen bg-background text-foreground">
    ...
    <aside className={`sidebar ${collapsed ? "collapsed" : ""} ${mobileOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}`}>
      <div className="flex items-center gap-3"><div className="brand-mark shrink-0"><Waves /></div>{!collapsed && <div className="min-w-0"><div className="brand-wordmark truncate text-lg leading-none">Jal<span className="text-primary">Setu</span></div><div className="eyebrow mt-1">Water intelligence</div></div>}<button className="ml-auto text-muted-foreground lg:hidden" onClick={() => setMobileOpen(false)} aria-label="Close menu"><X /></button></div>
      ...
      <div className={`flex items-center border-top-soft pt-5 ${collapsed ? "mt-5 justify-center" : "mt-5 gap-3"}`}><ProfileMenu align="start" />{!collapsed && <><div className="min-w-0"><div className="truncate text-xs font-medium">Tarun Oli</div><div className="truncate text-[10px] text-muted-foreground">User</div></div><Settings2 className="ml-auto size-4 text-muted-foreground" /></>}</div>
    ...
    <main className={`relative z-[1] transition-[padding] duration-300 ${collapsed ? "lg:pl-[84px]" : "lg:pl-[280px]"}`}><header className="topbar"><div className="flex items-center gap-3"><button className="text-muted-foreground lg:hidden" onClick={() => setMobileOpen(true)} aria-label="Open menu"><Menu /></button><span className={`brand-wordmark text-base leading-none ${collapsed ? "" : "lg:hidden"}`}>Jal<span className="text-primary">Setu</span></span><div className="hidden items-center gap-2 text-xs text-muted-foreground sm:flex"><span>Operations</span><span className="text-separator">/</span><span className="text-foreground">{active}</span></div></div><div className="flex items-center gap-3"><button onClick={() => setLive(!live)} className={`live-toggle ${live ? "on" : ""}`}><span className="status-dot" />{live ? "Live telemetry" : "Paused"}</button><button className="icon-button" aria-label="Notifications"><Bell /><span /></button><ThemeToggle /><ProfileMenu /></div></header>
...
function stdDev(arr: number[]) { const m = mean(arr); return Math.sqrt(mean(arr.map((v) => (v - m) ** 2))) }
function quantile(arr: number[], p: number) { const sorted = [...arr].sort((a, b) => a - b); const idx = (sorted.length - 1) * p; const lo = Math.floor(idx); const hi = Math.ceil(idx); if (lo === hi) return sorted[lo]; return sorted[lo] + (sorted[hi] - sorted[lo]) * (idx - lo) }
function pearson(a: number[], b: number[]) { const ma = mean(a); const mb = mean(b); const num = a.reduce((s, v, i) => s + (v - ma) * (b[i] - mb), 0); const den = Math.sqrt(a.reduce((s, v) => s + (v - ma) ** 2, 0) * b.reduce((s, v) => s + (v - mb) ** 2, 0)); return den === 0 ? 0 : num / den }
function correlationColor(v: number) { const t = (v + 1) / 2; const r = Math.round(220 - t * 60); const g = Math.round(90 + t * 30); const b = Math.round(200 - t * 170); const bg = v >= 0 ? `rgba(${Math.round(150 + v * 90)}, ${Math.round(70 - v * 20)}, ${Math.round(75 - v * 20)}, ${0.18 + Math.abs(v) * 0.5})` : `rgba(${Math.round(70 + v * 20)}, ${Math.round(110 + v * 30)}, ${Math.round(190 + v * 20)}, ${0.18 + Math.abs(v) * 0.5})`; return bg }

const metricCols: { key: "ph" | "turbidity" | "tds" | "temperature"; label: string; unit: string; decimals: number }[] = [
  { key: "ph", label: "ph", unit: "", decimals: 2 },
  { key: "turbidity", label: "turbidity", unit: " NTU", decimals: 2 },
  { key: "tds", label: "tds", unit: " mg/L", decimals: 1 },
  { key: "temperature", label: "temperature", unit: " °C", decimals: 1 },
]

function Analytics() {
  const ct = useChartTheme()
  const bars = locations.map((l, i) => ({ name: l.village.split(" ")[0], score: 92 - i * 4 + (i % 3) * 3 }))
  const series = metricCols.map((c) => ({ ...c, values: trend.map((t) => t[c.key]) }))
  const stats = series.map((s) => ({ label: s.label, count: s.values.length, mean: mean(s.values), std: stdDev(s.values), min: Math.min(...s.values), p25: quantile(s.values, 0.25), median: quantile(s.values, 0.5), p75: quantile(s.values, 0.75), max: Math.max(...s.values) }))
  const matrix = series.map((row) => series.map((col) => pearson(row.values, col.values)))
  return <div className="flex flex-col gap-6">
    <div className="grid gap-5 lg:grid-cols-[1.3fr_1fr]"><section className="glass-panel p-5 sm:p-6"><div className="flex items-center justify-between"><div><h2 className="text-base font-semibold">Network health score</h2><p className="mt-1 text-xs text-muted-foreground">Comparative confidence across active locations.</p></div><span className="status-chip border-primary/20 bg-primary/10 text-primary">94.8 avg</span></div><div className="mt-7 h-80"><ResponsiveContainer width="100%" height="100%"><BarChart data={bars}><CartesianGrid stroke={ct.grid} vertical={false} /><XAxis dataKey="name" tick={{ fill: ct.tick, fontSize: 10 }} axisLine={false} tickLine={false} /><YAxis domain={[0, 100]} tick={{ fill: ct.tick, fontSize: 10 }} axisLine={false} tickLine={false} /><Tooltip contentStyle={{ background: ct.tooltipBg, border: `1px solid ${ct.tooltipBorder}`, borderRadius: 10, fontSize: 11 }} /><Bar dataKey="score" fill="#63e6e2" radius={[6, 6, 0, 0]} /></BarChart></ResponsiveContainer></div></section><section className="glass-panel p-5 sm:p-6"><div className="eyebrow">Reference system</div><h2 className="mt-3 text-lg font-semibold">Safe water thresholds</h2><div className="mt-6 flex flex-col gap-4">{Object.entries(thresholds).map(([key, value]) => <div key={key} className="flex items-center justify-between border-bottom-soft pb-4 text-sm"><span className="capitalize text-muted-foreground">{key}</span><span className="font-mono text-primary">{value}</span></div>)}</div></section></div>

    <section className="glass-panel p-5 sm:p-6">
      <h2 className="text-lg font-semibold">Water quality metric distribution</h2>
      <div className="mt-6 grid gap-6 lg:grid-cols-[1.2fr_1fr]">
        <div>
          <div className="eyebrow">Parameter ranges and deviations</div>
          <div className="mt-4 overflow-x-auto"><table className="w-full min-w-[560px] text-left text-xs"><thead><tr className="border-bottom-soft text-muted-foreground">{["", "Count", "Mean", "Std Dev", "Min", "25%", "Median", "75%", "Max"].map((h) => <th key={h} className="whitespace-nowrap px-3 py-2 font-medium">{h}</th>)}</tr></thead><tbody>{stats.map((s) => <tr key={s.label} className="border-bottom-soft"><td className="whitespace-nowrap px-3 py-2.5 capitalize text-muted-foreground">{s.label}</td><td className="px-3 py-2.5 font-mono">{s.count.toFixed(2)}</td><td className="px-3 py-2.5 font-mono">{s.mean.toFixed(2)}</td><td className="px-3 py-2.5 font-mono">{s.std.toFixed(2)}</td><td className="px-3 py-2.5 font-mono">{s.min.toFixed(2)}</td><td className="px-3 py-2.5 font-mono">{s.p25.toFixed(2)}</td><td className="px-3 py-2.5 font-mono">{s.median.toFixed(2)}</td><td className="px-3 py-2.5 font-mono">{s.p75.toFixed(2)}</td><td className="px-3 py-2.5 font-mono">{s.max.toFixed(2)}</td></tr>)}</tbody></table></div>
        </div>
        <div>
          <div className="eyebrow">Correlation matrix of water attributes</div>
          <div className="mt-4 overflow-x-auto"><table className="w-full min-w-[360px] border-separate text-center text-[11px]" style={{ borderSpacing: 4 }}><thead><tr><th className="p-0" />{series.map((s) => <th key={s.label} className="px-2 py-1.5 font-medium capitalize text-muted-foreground">{s.label}</th>)}</tr></thead><tbody>{series.map((row, i) => <tr key={row.label}><th className="px-2 py-1.5 text-right font-medium capitalize text-muted-foreground">{row.label}</th>{series.map((_, j) => <td key={j} className="rounded-lg px-2 py-3 font-mono" style={{ background: correlationColor(matrix[i][j]) }}>{matrix[i][j].toFixed(2)}</td>)}</tr>)}</tbody></table></div>
        </div>
      </div>
    </section>

    <section className="glass-panel p-5 sm:p-6">
      <h2 className="text-lg font-semibold">Historical data table</h2>
      <div className="mt-4 max-h-96 overflow-y-auto overflow-x-auto"><table className="w-full min-w-[640px] text-left text-xs"><thead className="sticky top-0 bg-background/80"><tr className="border-bottom-soft text-muted-foreground">{["#", "Hour", "ph", "turbidity", "tds", "temperature"].map((h) => <th key={h} className="whitespace-nowrap px-3 py-2 font-medium capitalize">{h}</th>)}</tr></thead><tbody>{[...trend].reverse().map((row, i) => <tr key={row.hour + i} className="border-bottom-soft"><td className="px-3 py-2.5 text-muted-foreground">{trend.length - 1 - i}</td><td className="whitespace-nowrap px-3 py-2.5 font-mono text-muted-foreground">{row.hour}</td><td className="px-3 py-2.5 font-mono">{row.ph.toFixed(2)}</td><td className="px-3 py-2.5 font-mono">{row.turbidity.toFixed(2)} NTU</td><td className="px-3 py-2.5 font-mono">{row.tds.toFixed(1)} mg/L</td><td className="px-3 py-2.5 font-mono">{row.temperature.toFixed(1)} °C</td></tr>)}</tbody></table></div>
    </section>
  </div>
}
...
const objectives = [
  ...
  { icon: Activity, title: "Continuous telemetry", body: "A standardized data structure built for live IoT sensor pipelines, with offline sync as a fallback." },
]

const specSteps = [
  { code: "lib/aquasense-data.ts", body: "holds the current demo dataset and can be swapped for a live query layer without touching the UI." },
  { code: "app/api/chat/route.ts", body: "wires the assistant to a real-time model; point it at a production inference endpoint when ready." },
  { code: "components/dashboard.tsx", body: "reads metrics through typed props, so any new data source only needs to match the existing shape." },
]

function About() { return <section className="glass-panel max-w-3xl p-6 sm:p-8">
  <div className="flex items-center gap-4"><div className="brand-mark size-12"><Droplets /></div><div><h2 className="brand-wordmark text-3xl leading-none">JalSetu</h2><p className="mt-1.5 text-xs text-muted-foreground">Community-first water intelligence</p></div></div>
  <p className="mt-8 text-sm leading-7 text-muted-foreground">JalSetu turns distributed water sensors into a calm, actionable view of the communities they serve. This console monitors 48 locations across Northeast India, combining live telemetry, threshold checks, and explainable risk scoring.</p>
  <div className="mt-8 grid gap-3 sm:grid-cols-3">{[["48", "Locations monitored"], ["96%", "Sensors online"], ["24/7", "Telemetry coverage"]].map(([v, l]) => <div key={l} className="rounded-xl stat-card p-4"><div className="font-mono text-2xl text-primary">{v}</div><div className="mt-1 text-xs text-muted-foreground">{l}</div></div>)}</div>

  <div className="mt-10 border-top-soft pt-8"><h3 className="text-lg font-semibold">Core objectives</h3>
    <div className="mt-5 grid gap-4 sm:grid-cols-3">{objectives.map(({ icon: Icon, title, body }) => <div key={title} className="rounded-xl stat-card p-4"><div className="objective-icon"><Icon className="size-4" /></div><div className="mt-3 text-sm font-medium">{title}</div><p className="mt-1.5 text-xs leading-6 text-muted-foreground">{body}</p></div>)}</div>
  </div>

  <div className="mt-10 border-top-soft pt-8"><h3 className="text-lg font-semibold">Safe water quality parameters</h3>
    <p className="mt-2 text-xs leading-6 text-muted-foreground">Thresholds are aligned with WHO and Indian Standard drinking-water guidelines and drive every risk classification on this console.</p>
    <div className="mt-5 grid gap-3 sm:grid-cols-2">
      <div className="flex items-center justify-between rounded-xl stat-card p-4"><div><div className="text-sm font-medium">pH level</div><p className="mt-1 text-xs text-muted-foreground">Neither too acidic nor alkaline</p></div><span className="font-mono text-sm text-primary">{thresholds.ph}</span></div>
      <div className="flex items-center justify-between rounded-xl stat-card p-4"><div><div className="text-sm font-medium">Turbidity</div><p className="mt-1 text-xs text-muted-foreground">Cloudiness from suspended particles</p></div><span className="font-mono text-sm text-primary">{thresholds.turbidity}</span></div>
      <div className="flex items-center justify-between rounded-xl stat-card p-4"><div><div className="text-sm font-medium">Total dissolved solids</div><p className="mt-1 text-xs text-muted-foreground">Concentration of inorganic substances</p></div><span className="font-mono text-sm text-primary">{thresholds.tds}</span></div>
      <div className="flex items-center justify-between rounded-xl stat-card p-4"><div><div className="text-sm font-medium">Temperature</div><p className="mt-1 text-xs text-muted-foreground">Elevated readings accelerate pathogen growth</p></div><span className="font-mono text-sm text-primary">{thresholds.temperature}</span></div>
    </div>
  </div>

  <div className="mt-10 border-top-soft pt-8"><h3 className="text-lg font-semibold">Model &amp; data integration</h3>
    <p className="mt-2 text-xs leading-6 text-muted-foreground">This deployment runs on demo data. The interface is built on a modular structure so live sources can be dropped in without a rewrite:</p>
    <ol className="mt-5 flex flex-col gap-3">{specSteps.map(({ code, body }, i) => <li key={code} className="flex items-start gap-3 rounded-xl stat-card p-4"><span className="font-mono text-xs text-muted-foreground">{i + 1}</span><p className="text-xs leading-6 text-muted-foreground"><code className="code-chip">{code}</code> {body}</p></li>)}</ol>
  </div>
</section> }
