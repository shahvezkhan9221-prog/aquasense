...
export default function Dashboard() {
  ...
  return <div className="relative z-[1] min-h-screen bg-background text-foreground">
    ...
    <main className={`relative z-[1] transition-[padding] duration-300 ${collapsed ? "lg:pl-[84px]" : "lg:pl-[280px]"}`}><header className="topbar"><div className="flex items-center gap-3"><button className="text-muted-foreground lg:hidden" onClick={() => setMobileOpen(true)} aria-label="Open menu"><Menu /></button><span className="brand-wordmark text-base leading-none lg:hidden">Jal<span className="text-primary">Setu</span></span><div className="hidden items-center gap-2 text-xs text-muted-foreground sm:flex"><span>Operations</span><span className="text-separator">/</span><span className="text-foreground">{active}</span></div></div><div className="flex items-center gap-3"><button onClick={() => setLive(!live)} className={`live-toggle ${live ? "on" : ""}`}><span className="status-dot" />{live ? "Live telemetry" : "Paused"}</button><button className="icon-button" aria-label="Notifications"><Bell /><span /></button><ThemeToggle /><ProfileMenu /></div></header>
