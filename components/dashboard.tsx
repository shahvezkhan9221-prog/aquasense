...
import { Activity, AlertTriangle, BarChart3, Bell, ChevronDown, ChevronsLeft, ChevronsRight, CircleHelp, Droplets, Gauge, LayoutDashboard, MapPin, Menu, Moon, Radio, Search, Settings2, ShieldCheck, SlidersHorizontal, Sparkles, Sun, Waves, X, Zap } from "lucide-react"
...
export default function Dashboard() {
  ...
  return <div className="relative z-[1] min-h-screen bg-background text-foreground">
    ...
    <aside className={`sidebar ${collapsed ? "collapsed" : ""} ${mobileOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}`}>
      ...
      <div className={`flex items-center border-top-soft pt-5 ${collapsed ? "mt-5 justify-center" : "mt-5 gap-3"}`}><div className="avatar shrink-0" title={collapsed ? "Tarun Oli" : undefined}>TO</div>{!collapsed && <><div className="min-w-0"><div className="truncate text-xs font-medium">Tarun Oli</div><div className="truncate text-[10px] text-muted-foreground">Network administrator</div></div><Settings2 className="ml-auto size-4 text-muted-foreground" /></>}</div>
