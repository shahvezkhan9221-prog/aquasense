...
import { Activity, AlertTriangle, BarChart3, Bell, ChevronDown, CircleHelp, Droplets, Gauge, LayoutDashboard, MapPin, Menu, Moon, Radio, Search, Settings2, ShieldCheck, SlidersHorizontal, Sparkles, Sun, Waves, X, Zap } from "lucide-react"
...
const nav = [{ label: "Overview", icon: LayoutDashboard }, { label: "Analytics", icon: BarChart3 }, { label: "Risk Simulator", icon: SlidersHorizontal }, { label: "About AquaSense", icon: CircleHelp }]
...
function Guardian({ risk, page }: { risk: "LOW" | "MEDIUM" | "HIGH"; page: string }) {
  ...
  if (dismissed) return <button className="guardian-avatar-btn guardian-inline-avatar" onClick={() => setDismissed(false)} aria-label="Show Neer, the water guardian"><img src="/images/neer-guardian.png" alt="Neer, the AquaSense water guardian mascot" /></button>
  return <div className={`guardian-widget guardian-inline tone-${tone}`}>
    <div className={`guardian-inline-avatar trick-${trick}`}>
      <img src="/images/neer-guardian.png" alt="Neer, the AquaSense water guardian mascot, a cartoon warrior fighting water-borne pathogens" onClick={doTrick} />
...
export default function Dashboard() {
  ...
  return <div className="relative z-[1] min-h-screen bg-background text-foreground">
    ...
    <aside className={`sidebar ${mobileOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}`}>
      ...
      <nav className="mt-4 flex flex-col gap-1">{nav.map(({ label, icon: Icon }) => <button key={label} onClick={() => changeView(label)} className={`nav-item ${active === label ? "active" : ""}`}><Icon />{label}{active === label && <span className="ml-auto size-1.5 rounded-full bg-primary shadow-[0_0_10px_#63e6e2]" />}</button>)}</nav>
      ...
      <div className="mt-5 flex items-center gap-3 border-top-soft pt-5"><div className="avatar">AK</div><div className="min-w-0"><div className="truncate text-xs font-medium">Aarav Khanna</div><div className="truncate text-[10px] text-muted-foreground">Network administrator</div></div><Settings2 className="ml-auto size-4 text-muted-foreground" /></div>
    </aside>
